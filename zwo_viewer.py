...
class ZWOWorkoutVisualizer:
    ...
    def create_power_profile(self, segments):
        ...
        for segment in segments:
            ...
            if segment['type'] == 'warmup':
                ...
                avg_power = (segment['power_start'] + segment['power_end']) / 2 * self.ftp
                power_points.extend([segment['power_start'] * self.ftp, segment['power_end'] * self.ftp])
    ...
    def plot_zwo_workout(self, zwo_path: str, save_path: str = None, show_plot: bool = True):
        ...
        if show_plot:
            ...
            for segment in segments:
                ...
                ax_power.axvspan(start_min, end_min, alpha=0.2, color=segment_color, zorder=1)
            ...
            # Plot step timeline
            bars = ax_steps.barh(range(len(segments)), durations, left=starts, 
            ...
            ax_steps.set_yticklabels([f"Step {i+1}" for i in range(len(segments))], fontsize=8)
        ...
        for i, segment in enumerate(segments):
            segment_type = self.intensity_names.get(segment['type'], segment['type'].title())
...
def main():
    ...
    visualizer.plot_zwo_workout(args.file, save_path=args.output, show_plot=not args.no_show)
