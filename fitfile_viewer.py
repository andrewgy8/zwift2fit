...
class FITWorkoutVisualizer:
    ...
    def create_power_profile(self, segments: List[Dict]) -> Tuple[np.ndarray, np.ndarray, List[Dict]]:
    ...
    def plot_fit_workout(self, workout_info: Dict[str, Any], save_path: str = None, show_plot: bool = True):
        ...
        if show_plot:
            ...
            fig, (ax_power, ax_steps) = plt.subplots(2, 1, figsize=(14, 10), 
                                                    gridspec_kw={'height_ratios': [2, 1]})
            
            # Create power profile data
            time_data, power_data, range_info = self.create_power_profile(workout_info['segments'])
            time_minutes = time_data / 60
            
            # Plot power profile
            ax_power.plot(time_minutes, power_data, 'k-', linewidth=2.5, label='Target Power', zorder=3)
            ...
            for range_data in range_info:
                ...
                intensity_color = self.intensity_colors.get(range_data['intensity'], '#808080')
                
                ax_power.fill_between([start_min, end_min], 
                                    [range_data['low_power'], range_data['low_power']], 
                                    [range_data['high_power'], range_data['high_power']], 
                                    alpha=0.3, color=intensity_color, zorder=2)
            ...
            for segment in workout_info['segments']:
                ...
                intensity_color = self.intensity_colors.get(segment['intensity'], '#808080')
                
                if segment['power_target']:
                    power = segment['power_target']
                else:
                    power = self.ftp * 0.5
                
                ax_power.axvspan(start_min, end_min, alpha=0.2, color=intensity_color, zorder=1)
            
            # Add FTP reference line
            ax_power.axhline(y=self.ftp, color='red', linestyle='--', alpha=0.7, 
                           label=f'FTP ({self.ftp}W)', zorder=1)
            ...
            if len(power_data) > 0:
                ...
                if range_info:
                    max_power = max(max_power, max([r['high_power'] for r in range_info]))
            ...
            # Plot step timeline
            bars = ax_steps.barh(range(len(workout_info['segments'])), durations, left=starts, 
            ...
            for i, (bar, label) in enumerate(zip(bars, labels)):
                ...
                if width > 1:  # Only show label if segment is wide enough (1 minute)
                    ax_steps.text(bar.get_x() + width/2, bar.get_y() + bar.get_height()/2, 
            ...
            ax_steps.set_yticklabels([f"Step {i+1}" for i in range(len(workout_info['segments']))], fontsize=8)
        ...
        for i, segment in enumerate(workout_info['segments']):
            ...
            if segment['power_range']:
                power_str = f"{segment['power_range'][0]:.0f}-{segment['power_range'][1]:.0f}W"
    ...
    for fit_file in args.files:
        ...
        if workout_info['segments']:
            ...
            if len(args.files) > 1 and args.output:
                ...
                save_path = f"{base}_{os.path.basename(fit_file).replace('.fit', '')}{ext}"
            
            visualizer.plot_fit_workout(workout_info, save_path=save_path, show_plot=not args.no_show)
        else:
