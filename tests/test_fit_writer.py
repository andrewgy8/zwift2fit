...
class TestFieldTypes:
    ...
    def test_unsupported_field_type(self, tmp_path):
        ...
        with pytest.raises(
            IOError, match="Error writing FIT file.*Unsupported field type"
        ):
            writer.write_fit_file(str(temp_path))
...
class TestEdgeCases:
    ...
    def test_maximum_values(self):
        ...
        max_uint32 = 2**32 - 1
        writer.add_workout_step(
            0, "Max Values", 0, max_uint32, max_uint32, max_uint32, 255
        )
...
class TestIntegration:
    ...
    def test_realistic_workout_file(self, tmp_path):
        ...
        for i in range(5):
            # Work interval
            writer.add_workout_step(
                i * 2 + 1, f"Interval {i + 1}", 0, 240000, 250, 280, 0
            )
            # Rest interval
            writer.add_workout_step(
                i * 2 + 2, f"Recovery {i + 1}", 0, 120000, 80, 120, 1
            )
...
class TestCreateWorkoutFile:
    ...
    def test_create_workout_file_single_segment(self, tmp_path):
        ...
        writer = FITFileWriter()
        segments = [WorkoutSegment(type="steady", duration=300, power=0.75)]
        temp_path = tmp_path / "single.fit"

        crc = writer.create_workout_file(
            segments, str(temp_path), "Single Test", ftp=250
        )
    ...
    def test_create_workout_file_warmup_segment(self, tmp_path):
        ...
        writer.create_workout_file(
            segments, str(temp_path), "Warmup Test", ftp=280
        )
    ...
    def test_create_workout_file_cooldown_segment(self, tmp_path):
        ...
        segments = [
            WorkoutSegment(
                type="cooldown", duration=600, power_start=0.7, power_end=0.4
            )
        ]
        temp_path = tmp_path / "cooldown.fit"

        writer.create_workout_file(segments, str(temp_path), "Cooldown Test")
    ...
    def test_create_workout_file_interval_segments(self, tmp_path):
        ...
        writer.create_workout_file(segments, str(temp_path), "Interval Test")
    ...
    def test_create_workout_file_unknown_segment_type(self, tmp_path):
        ...
        writer = FITFileWriter()
        segments = [WorkoutSegment(type="unknown_type", duration=300, power=0.8)]
        temp_path = tmp_path / "unknown.fit"

        writer.create_workout_file(segments, str(temp_path), "Unknown Test")
    ...
    def test_create_workout_file_complex_workout(self, tmp_path):
        ...
        segments = [
            WorkoutSegment(
                type="warmup", duration=600, power_start=0.5, power_end=0.75
            ),
        ...
        writer.create_workout_file(
            segments, str(temp_path), "Complex Workout", ftp=300
        )
    ...
    def test_create_workout_file_clears_existing_data(self, tmp_path):
        ...
        writer.create_workout_file(segments, str(temp_path), "New Workout")
...
class TestCalculateFTPTargets:
    ...
    def test_power_range_warmup(self):
        ...
        target_low, target_high = calculate_ftp_targets(
            0.5, ftp=280, power_high_fraction=0.75
        )
    ...
    def test_power_range_cooldown(self):
        ...
        target_low, target_high = calculate_ftp_targets(
            0.4, ftp=250, power_high_fraction=0.7
        )
    ...
    def test_power_range_same_values(self):
        ...
        target_low2, target_high2 = calculate_ftp_targets(
            0.6, ftp=280, power_high_fraction=0.6
        )
    ...
    def test_power_range_inverted(self):
        ...
        target_low, target_high = calculate_ftp_targets(
            0.8, ftp=250, power_high_fraction=0.6
        )
    ...
    def test_real_world_examples(self):
        ...
        target_low, target_high = calculate_ftp_targets(
            0.44, ftp=280, power_high_fraction=0.75
        )
        assert isinstance(target_low, int)
        assert isinstance(target_high, int)
