...
class TestWorkout:
    ...
    def test_basic_workout_creation(self):
        ...
        segments = [
            ...
            WorkoutSegment(type='cooldown', duration=180, power_start=0.6, power_end=0.3)
        ]
        workout = Workout(name='Test Workout', description='Test Description', segments=segments)
...
class TestFileBasedParsing:
    ...
    def test_parse_interval_workout(self, test_files_dir):
        ...
        interval_work_segments = [s for s in workout.segments if s.type == 'interval_work']
        interval_rest_segments = [s for s in workout.segments if s.type == 'interval_rest']
...
class TestErrorHandling:
    ...
    def test_missing_workout_element(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.zwo', delete=False) as f:
            f.write('<?xml version="1.0"?><workout_file><name>Test</name></workout_file>')
            f.flush()
...
class TestIntegration:
    ...
    def test_performance_with_large_intervals(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.zwo', delete=False) as f:
            ...
            f.write('<workout>')
            f.write('<IntervalsT Repeat="100" OnDuration="10" OffDuration="5" OnPower="1.0" OffPower="0.5"/>')
            f.write('</workout>')
            f.write('</workout_file>')
            f.flush()
