...
class WorkoutComparator:
    ...
    def compare_workouts(self, zwo_path: str, fit_path: str, save_path: str = None, show_plot: bool = True):
        ...
        ax_zwo_power.plot(zwo_time_data / 60, zwo_power_data, 'b-', linewidth=2.5, label='ZWO Power Profile')
        ...
        ax_fit_power.plot(fit_time_data / 60, fit_power_data, 'g-', linewidth=2.5, label='FIT Power Profile')
        ...
        for segment in fit_segments:
            ...
            ax_fit_power.axvspan(start_min, end_min, alpha=0.2, color=segment_color, zorder=1)
        
        ax_fit_power.axhline(y=self.ftp, color='red', linestyle='--', alpha=0.7, label=f'FTP ({self.ftp}W)')
        ax_fit_power.set_ylabel('Power (W)', fontsize=12)
        ax_fit_power.set_title(f'FIT: {fit_workout["name"]} ({fit_total_duration//60}:{fit_total_duration%60:02d})', 
                              fontsize=14, fontweight='bold')
        ...
        ax_zwo_timeline.set_title(f'ZWO Steps ({len(zwo_segments)} segments)', fontsize=12, fontweight='bold')
        ax_zwo_timeline.set_yticks(range(0, len(zwo_segments), max(1, len(zwo_segments)//10)))
        ...
        bars_fit = ax_fit_timeline.barh(range(len(fit_segments)), fit_durations, left=fit_starts, 
                                       color=fit_colors, alpha=0.8, edgecolor='black', linewidth=0.5)
        
        ax_fit_timeline.set_xlabel('Time (minutes)', fontsize=12)
        ax_fit_timeline.set_ylabel('Steps', fontsize=12)
        ax_fit_timeline.set_title(f'FIT Steps ({len(fit_segments)} segments)', fontsize=12, fontweight='bold')
        ax_fit_timeline.set_yticks(range(0, len(fit_segments), max(1, len(fit_segments)//10)))
        ...
        duration_match = "✓" if abs(zwo_total_duration - fit_total_duration) <= 60 else "✗"
        ...
        for i in range(min(8, len(zwo_segments), len(fit_segments))):
            ...
            if fit_seg['power_range']:
                fit_power_str = f"{fit_seg['power_range'][0]:.0f}-{fit_seg['power_range'][1]:.0f}W"
            else:
        ...
        ax_comparison.text(0.02, 0.98, comparison_text, transform=ax_comparison.transAxes, 
                          fontsize=9, verticalalignment='top', fontfamily='monospace')
        
        # Add file sources
        fig.text(0.02, 0.02, f"ZWO: {os.path.basename(zwo_path)} | FIT: {os.path.basename(fit_path)}", 
...
def main():
    parser = argparse.ArgumentParser(description='Compare ZWO and FIT workout files side by side')
    parser.add_argument('zwo_file', help='ZWO file to compare')
    parser.add_argument('fit_file', help='FIT file to compare')
    parser.add_argument('--ftp', type=int, default=280, help='Functional Threshold Power in watts (default: 280)')
    ...
    comparator.compare_workouts(args.zwo_file, args.fit_file, 
                               save_path=args.output, show_plot=not args.no_show)
