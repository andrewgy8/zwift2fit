...
def _parse_warmup(element: ET.Element) -> WorkoutSegment:
    ...
    return WorkoutSegment(
        type="warmup", duration=duration, power_start=power_low, power_end=power_high
    )
...
def _parse_steady_state(element: ET.Element) -> WorkoutSegment:
    ...
    return WorkoutSegment(type="steady", duration=duration, power=power)
...
def _parse_cooldown(element: ET.Element) -> WorkoutSegment:
    ...
    return WorkoutSegment(
        type="cooldown", duration=duration, power_start=power_low, power_end=power_high
    )
...
def _parse_intervals_t(element: ET.Element) -> List[WorkoutSegment]:
    ...
    for i in range(repeat):
        # Add work interval
        segments.append(
            WorkoutSegment(type="interval_work", duration=on_duration, power=on_power)
        )
        ...
        if i < repeat - 1 or off_duration > 0:
            segments.append(
                WorkoutSegment(
                    type="interval_rest", duration=off_duration, power=off_power
                )
            )
...
def parse_zwo_to_workout(zwo_path: str) -> Workout:
    ...
    try:
        ...
        return Workout(name=name, description=description, segments=segments)
